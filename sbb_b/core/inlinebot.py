...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**sbb_b"):
            buttons = [
                (
                    ...
                    Button.url("DEV", "https://t.me/Drago_dr"),
                )
            ]
            ...
            result = builder.article(
                title="sbb_b Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب دراكو **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/no1bros"),
                Button.url(
                    "المطور",
                    "t.me/ssxhh",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل سيثـون كرستالمن نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[SYTHON CRYSTAL 𝙐𝙎𝙀𝙍𝘽𝙊𝙏 🧸♥](https://t.me/no1bros)",
