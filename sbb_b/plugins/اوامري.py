...
ROE = "** هـذه هي قائمة اوامـر سـورس سيثـون كرستال **"
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("اوامري") and event.query.user_id == sbb_b.uid:
            buttons = [
                [Button.inline("‹ معلومات سيثون كرستال ›", data="AOMRDB")],
                [
...
اوامر فارات سورس سيثون كرستال @no1bros""",
        buttons=[
...
async def _(event):
    butze = [
                ...
                [
                    Button.inline("❲ تحويل لكتاب ❳", data="LTABAMEKF"),
...
async def _(event):
    ...
    Button.inline("❲ ايدي ❳", data="ALIVEDIII"),
    ],
    [
