...
async def _(event):
    await event.edit(
        """
 ‹ اوامر الصيد الخاصة بسورس سيثون كرستال › 
...
‹ الامر︙ `تثبيت` + معرف ›
*  ‹ وظيفة الامر : يقوم بالتثبيت على المعرف عندما يصبح متاح يأخذ ›
        ...
        try:
            ch = await sbb_b(
                functions.channels.CreateChannelRequest(
                    title="‹ kepthon HUNTER - صيد سيثون كرستال›",
                    about="This channel to hunt username by - @kepthon ",
                )
            )
...
async def _(event):
    ...
    try:
        ...
        try:
            ...
            ch = ch.updates[1].channel_id
            await event.edit(f"** ‹ بنجاح بدأ التثبيت ›**")
        except Exception as e:
            await sbb_b.send_message(
                event.chat_id, f" ‹ خطأ في انشاء القناة , الخطأ : {stre)}    ›"
            )
    isauto.clear()
    isauto.append("on")
    username = str(msg[1])
...
async def _(event):
    if "on" in isclaim:
        await event.edit(f"**‹ الصيد وصل لـ({trys[0]}) **من المحاولا ›ت")
    elif "off" in isclaim:
