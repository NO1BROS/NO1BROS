...
async def _(event):
    ...
    await edit_delete(event, "**- تم بنجاح انتحال حساب المستخدم السورس @no1bros**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#الانتحال\nتم انتحال المستخدم: [{first_name}](tg://user?id={user_id }) السورس @no1bros ",
        )
...
async def revert(event):
    ...
    await edit_delete(event, "**- تم بنجاح ارجاع الحساب الى وضعه الاصلي السورس @no1bros**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "#الاعادة\nتم بنجاح اعادة الحساب الى وضعه السابق السورس @no1bros",
        )