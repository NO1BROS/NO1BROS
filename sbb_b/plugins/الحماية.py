...
async def on_new_private_message(event):
    ...
    if event.chat_id == 5298061670:
        await event.client.send_message(chat, "- اهلا بك يا مطوري العزيز ")
...
async def on_plug_in_callback_query_handler(event):
    ...
    try:
        ...
        PM_WARNS = {}  # ترجمه وكتابة فريق كيبثـون
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
@sbb_b.ar_cmd(pattern="الحماية (تشغيل|تعطيل)$")  # ترجمه وكتابة فريق كيبثـون
...
async def pmpermit_on(event):
    input_str = event.pattern_match.group(1)
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None:  # ترجمه وكتابة فريق كيبثـون
...
async def approve_p_m(event):
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة الحماية سيثـون كرستال.txt",
        caption="قائمة المسموح لهم الحالية\n سورس سيثـون كرستال العربي \n @no1bros",
    )  # ترجمه وكتابة فريق كيبثـون