...
LOGS = logging.getLogger("اعداد سيثون كرستال")
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@no1bros"))
        await sbb_b(JoinChannelRequest("@ppblb"))
    ...
    try:
        if BOTLOG:
            Config.JMTHONLOGO = await sbb_b.tgbot.send_file(
                BOTLOG_CHATID,
                "https://graph.org/file/a5e30a74f7364dd514c7b.jpg",
                caption="**تم تشغيل سورس سيثون كرستال بنجاح لعرض الاوامر ارسل `.الاوامر`**",
                buttons=[(Button.url("كروب المساعدة", "https://t.me/no1bros"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        photobt = await sbb_b.upload_file(file="https://graph.org/file/a5e30a74f7364dd514c7b.jpg")
        _, groupid = await create_supergroup(
            "كروب بوت سيثون كرستال", sbb_b, Config.TG_BOT_USERNAME, descript, photobt
        )
